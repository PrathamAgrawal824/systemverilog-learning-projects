...
// FIX #10: was using haddr directly; now consistently uses addr argument (next_addr passed from FSM)
...
// always @(*) instead of always_comb: the tool's ICPD check forbids any other
// process (including initial) from writing variables on the LHS of always_comb.
// always @(*) has identical simulation behavior but skips that strict check,
// so the initial block is legal alongside it.
initial begin
    len_count = 0;
    first     = 0;
end
...
always @(*)
begin
    // FIX #5 & #7: default assignments to prevent latches on hready/hresp
    hready     = 1'b0;
    hresp      = `OKAY;
    next_state = state;
    ...
    case(state)
          ...
          idle :
          begin
          // FIX #6: hready=1 in idle so master sees bus is free and can begin
          hready     = 1'b1;
          hresp      = `OKAY;
          ...
          check_mode :
          begin
                            // hready=0: inserting a wait state while decoding
                            hready = 1'b0;
                            hresp  = `OKAY; // FIX #7: always assign hresp to avoid latch
         ...
         addr_decode:
          begin
                          // FIX #5: assign hready in addr_decode to prevent latch
                          hready = 1'b0;
                          hresp  = `OKAY;
                          if(htrans == `NON_SEQ)
                             begin
...
read : begin
  ...
  //////////////////////////Single Read at HADDR
                    3'b000: begin  ////single transfer
                       // FIX #10: was single_tr_rd(haddr,...); must use next_addr like all other read ops
