...
class generator;
  ...
  task run();
    repeat(count) begin
      ...
      mbxgd.put(tr.copy);
      // FIX #9: only send ulen to monitor mailbox for unspec-length burst (hburst==3'b001)
      // For all other burst types the monitor does not consume from mbxgm,
      // so unconditionally putting here would cause the mailbox to fill and block the generator.
      if(tr.hburst == 3'b001)
        mbxgm.put(tr.ulen);
      @(drvnext);
      @(sconext);
...
class monitor;
  ...
  virtual ahb_if vif;
  // FIX #8: tr is now a local variable created fresh per beat inside each task,
  // so each mbxms.put() delivers an independent object to the scoreboard.
  ...
  task run();
    forever begin
      ...
      // FIX #1: changed second 'if' to 'else if' so write and read paths
      // are mutually exclusive each clock cycle. Without this, after a write
      // burst task returns the signals may have changed and the read check
      // fires spuriously on stale/updated hwrite==0 from the just-finished write.
      else if(vif.hresetn && vif.hsel && (vif.hwrite == 0))
        begin
        case(vif.hburst)
          3'b000: single_tr_rd();
          3'b001: unspec_tr_rd();
